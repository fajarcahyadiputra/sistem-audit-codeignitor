...
class Ion_auth_model extends CI_Model
{
	...
	public function register($identity, $password, $email, $additional_data = [], $groups = [])
	{
		...
		if (!empty($groups))
		{
			...
			{
				$this->add_to_group($additional_data['group_id'], $id);
			}
		}
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select("a.$this->identity_column, a.email, a.phone, a.id, a.password, a.active, a.last_login, c.name")
						  ->from("users a")
						  ->join('users_groups b', 'b.user_id=a.id', 'left')
						  ->join('groups c', 'c.id=b.group_id', 'left')
	...
	public function login_remembered_user()
	{
		...
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', id, email, remember_code, last_login, phone')
